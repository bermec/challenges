...
text = "If it will feed nothing else, it will feed my revenge. He hath disgrac'd me and hind'red me\
half a million; laugh'd at my losses, mock'd at my gains, scorned my nation, thwarted my bargains,\
...
you in the rest, we will resemble you in that. If a Jew wrong a Christian, what is his humility? Revenge.\
...
for x in range(0, len(sentence)):
    sentence[x] = sentence[x].split(' ')
    if len(sentence[x]) > acc:
        acc = len(sentence[x])
        long_str = sentence[x]


print('Longest sentence is {0} words long'.format(acc))
print(' '.join(long_str))