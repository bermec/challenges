...
with open(file, 'r') as f:
    ...
    #    #if extract[0] == '\\':
#            #print('Invalid string (dosn\'t end)')
    #    #elif extract[0] not in escape_chars:
            #print('Invalid string, bad escape code ' + extract[0])
        #else:
            #print(line)

        for x in range(0, len(extract)):
            temp = extract[x]
            if extract[x] not in escape_chars:
                print('Invalid string' + extract[x])
                break
        print(line)
