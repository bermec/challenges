...
for candidate in candidates:
    ...
    if candidate[2] == visitor_in:
        ...
        for candidate in candidates:
            ...
            if (candidate[0] == visitor) and (candidate[2] == visitor_out):
                out_time = candidate[3]
                visitor_time = float(out_time) - float(in_time)
                room_visitor_length[visitor] = room_visitor_length.get(visitor, 0) + visitor_time


print(room_visitor_length)
room_list = []
for k,v in room_visits.items():
    room_list.append(k)

print('Room ' + str(room_list[0]) + ', ' + str(int(room_visitor_length['1'])) + ' minute average visit, ' + str(room_visits['0']) + ' visitor total')